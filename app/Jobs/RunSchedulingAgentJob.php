...
class RunSchedulingAgentJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(
        ...
        try {
            $agent = SchedulingAgent::make()
                ->resumeForBackgroundRun($run->getConversationId(), $run->getUserMessageId());
            ...
            $linker->linkProposalsToLatestAssistantMessage($run->getConversationId());
            $linker->linkQuestionsToLatestAssistantMessage($run->getConversationId());
    ...
    private function persistAssistantMessage(AgentRun $run, Collection $events, string $assistantContent, User $user): string
    {
        ...
        ConversationMessage::query()->create([
            ...
            'tool_results' => $events
                ...
                ->values()
                ->all(),
            'usage' => StreamEnd::combineUsage($events)->toArray(),
            'meta' => [],
        ]);
