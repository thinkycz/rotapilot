...
class PasswordUpdateController extends AutomaticController
{
    ...
    public function __invoke(ApiFormRequest $request): SymfonyResponse
    {
        ...
        $newPassword = $validated->assertString('new_password');
        $confirmation = $validated->assertString('new_password_confirmation');

        if ($newPassword !== $confirmation) {
            $request->thrower()
                ->error('new_password_confirmation', 'auth.password_mismatch')
                ->throw();
        }
    ...
    protected function validate(ApiFormRequest $request): Parser
    {
        ...
        return $request->builder()
            ->rules([
                ...
                'new_password_confirmation' => $authValidity->password()->required(),
            ])
