...
class PasswordResetController extends AutomaticController
{
    ...
    public function __invoke(ApiFormRequest $request): SymfonyResponse
    {
        ...
        $newPassword = $validated->assertString('password');
        $confirmation = $validated->assertString('password_confirmation');

        if ($newPassword !== $confirmation) {
            $request->thrower()
                ->error('password_confirmation', 'auth.password_mismatch')
                ->throw();
        }
